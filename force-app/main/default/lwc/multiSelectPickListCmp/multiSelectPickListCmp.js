...
export default class MultiSelectPickListCmp extends LightningElement {
	...
	connectedCallback() {
		...
		this.loadComponent(optionData, value, values);
	}

	loadComponent(optionData, value, values) {
		if (value || values) {
			var searchString;
			var count = 0;
	...
	@api
	refreshSelectedValues(selectedValues) {
		this.selectedValues = selectedValues;
		this.showDropdown = false;
		var optionData = this.options ? JSON.parse(JSON.stringify(this.options)) : null;
		var value = this.selectedValue ? JSON.parse(JSON.stringify(this.selectedValue)) : null;
		var values = selectedValues ? JSON.parse(JSON.stringify(selectedValues)) : null;
		this.loadComponent(optionData, value, values);
	}
	...
	refreshOptions(options) {
		...
		this.showDropdown = false;
		var optionData = options ? JSON.parse(JSON.stringify(options)) : null;
		var value = this.selectedValue ? JSON.parse(JSON.stringify(this.selectedValue)) : null;
		var values = this.selectedValues ? JSON.parse(JSON.stringify(this.selectedValues)) : null;
		this.loadComponent(optionData, value, values);
	}
